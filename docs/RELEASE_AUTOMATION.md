...
일반 개발 작업은 전체 검증 뒤 변경사항을 커밋하고 `codex/**` 브랜치에 푸시한다. 푸시하면 `Internal Test`가 자동 실행되어 웹 산출물과 Android 테스트 APK를 만들고, 운영 DB 변경 없이 현재 마이그레이션 상태와 임시 학생 PC·휴대폰 동기화를 확인한다.
