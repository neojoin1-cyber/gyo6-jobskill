...
function careerSnapshot(draft) {
  return draft?.careerProfileSnapshot || draft?.sourceCover?.careerProfileSnapshot || null
}
      ...
      <div className="cover-review-layout">
        ...
        {!selected ? <section className="cover-review-no-selection"><FileText /><b>첨삭할 글을 선택하세요</b></section> : <>
          <section className="cover-review-document">
            ...
            {careerSnapshot(selected.draft) && <section className="cover-review-career-snapshot"><header><div><span>제출 시점 취업 준비정보</span><b>{careerSnapshot(selected.draft).currentGrade}학년 · {careerSnapshot(selected.draft).departmentName || '학과 미입력'} · {careerSnapshot(selected.draft).targetRole || '관심 직무 미입력'}</b></div><strong>{careerSnapshot(selected.draft).readiness?.score || 0}<small>/100</small></strong></header><div>{(careerSnapshot(selected.draft).qualificationSummary || []).map(item => <span key={item}>자격 · {item}</span>)}{(careerSnapshot(selected.draft).activitySummary || []).map(item => <span key={item}>활동 · {item}</span>)}</div>{careerSnapshot(selected.draft).readiness?.gaps?.length > 0 && <p>우선 보완 · {careerSnapshot(selected.draft).readiness.gaps[0]}</p>}</section>}
