...
function CoverWritingMock({ onGoLearn, onGoPractical }) {
  ...
  if (!started) return <section className="cover-writing-mock-setup">
    ...
    {prompt && <article className="cover-writing-mock-preview"><span>{prompt.label}</span><p>{prompt.question}</p><small>평가 요소 · {prompt.required.join(' · ')}</small></article>}
...
function CoverKnowledgeDiagnostic({ onGoLearn }) {
  ...
  if (!started) {
    return <section className="cover-assessment-setup">
      <header><span>WRITING CHECK-UP</span><h3>작성 기준 진단</h3><p>질문 의도부터 최종 점검까지 부족한 기준을 찾음</p></header>
  ...
  if (finished) {
    ...
    return <section className="cover-assessment-result">
      <header className={score >= 80 ? 'is-good' : 'is-review'}><CheckCircle weight="fill" /><div><span>작성 기준 진단 결과</span><strong>{score}점</strong><p>{correct}/{questions.length}문항 정답</p></div></header>
  ...
  return <section className="cover-assessment-run">
    ...
    {selected !== undefined && <div className={`cover-diagnostic-explain ${selected === current.answer ? 'is-correct' : 'is-wrong'}`}><b>{selected === current.answer ? '기준 이해' : `정답 ${current.answer + 1}번`}</b><p>{current.explanation}</p></div>}
