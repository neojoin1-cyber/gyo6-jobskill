...
function CoverLetterBuilder({ initialWorkspace = 'learn', initialEvidenceSeed: linkedEvidenceSeed, initialCareerProfile: linkedCareerProfile, onLearningContext }) {
  ...
  return (
    <div className="cover-builder">
      ...
      {step.id === 'major' && <section className="cover-evidence-entry"><ClipboardText weight="duotone" /><div><b>문항에 맞는 경험 근거 고르기</b><p>저장한 실습·프로젝트 경험을 고른 뒤 다음 단계에서 내 행동과 결과를 구체화합니다.</p></div><button onClick={() => setWorkspace('evidence')}>{evidenceBank.length ? `근거 ${evidenceBank.length}장 보기` : '근거 만들기'}<CaretRight /></button></section>}
