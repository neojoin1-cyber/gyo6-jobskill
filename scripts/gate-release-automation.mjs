...
const requiredFiles = [
  '.github/workflows/closed-test.yml',
  ...
  'scripts/release/build-closed-aab.mjs',
  'scripts/release/publish-app-config.mjs',
  'scripts/release/finalize-closed-release.mjs',
  '.github/workflows/closed-release-finalize.yml',
...
for (const name of ['release:migrations', 'release:sync', 'release:android', 'release:verify', 'closed:aab', 'closed:test', 'closed:publish-config', 'closed:finalize']) {
...
for (const token of ['production-approval:', 'OPERATIONS_APPROVED', 'supabase link --project-ref', 'remote-preflight:', 'android-preflight:', 'release:migrations -- --apply', 'release:sync', 'jobhigh-ci-test.jks', 'android/keystore.properties', 'release:android -- --skip-web-build', 'needs: [remote-preflight, android-preflight]']) {
  requireText(workflow, token, `GitHub 출시 게이트 누락: ${token}`)
}

const closed = read('.github/workflows/closed-test.yml')
for (const token of ['"codex/**"', 'supabase link --project-ref', 'release:sync', 'jobhigh-ci-test.jks', 'android/keystore.properties', 'release:android -- --skip-web-build', 'actions/upload-artifact@v4']) {
  requireText(closed, token, `GitHub 비공개 테스트 게이트 누락: ${token}`)
}
...
for (const token of ['latest_build', 'latest_version', "projects', 'api-keys'", 'force-minimum']) {
  requireText(publishConfig, token, `비공개 테스트 업데이트 안내 자동화 누락: ${token}`)
}
...
const finalizeWorkflow = read('.github/workflows/closed-release-finalize.yml')
for (const token of ['workflow_dispatch:', 'play_published:', 'closed:publish-config', 'SUPABASE_ACCESS_TOKEN']) {
  requireText(finalizeWorkflow, token, `비공개 테스트 업데이트 안내 워크플로 누락: ${token}`)
}
