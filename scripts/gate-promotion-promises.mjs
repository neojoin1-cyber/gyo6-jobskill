...
for (const label of ['연결됨', '화면 벗어남', '연결 확인', '미접속']) check(teacherLearningSource.includes(label), `수업 연결 상태 ${label} 표시 누락`)
check(teacherLearningSource.includes('상세보기') && teacherLearningSource.includes('현재 위치 다시 보내기'), '수업 연결 상세 또는 학생 앱 위치 재전달 기능 누락')
