...
for (const e of entries) {
  ...
  if (choices.length < 2) { unusable.push({ id, subject: e.subject, reason: 'choices_lt_2', stem: stemOf(q).slice(0, 60) }); continue }
  seen.add(id)
...
writeFileSync('verification/adversarial-4.8.13/handoff/CNT_blind_questions.json',
  JSON.stringify({
    _meta: meta,
...
writeFileSync('verification/adversarial-4.8.13/private/CNT_app_answers.json',
  JSON.stringify({
...
writeFileSync('verification/adversarial-4.8.13/private/CNT_unusable.json',
  JSON.stringify({ unusable, duplicateIds: dupIds }, null, 1) + '\n')
